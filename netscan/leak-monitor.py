...
def scan_github(db):
    ...
    if not gh_token:
        ...
        if not gh_token:
            log("  No GITHUB_TOKEN — skipping code search (needs auth)")
            log("  Set GITHUB_TOKEN in ~/.openclaw/.env for code dorks")
            return 0
...
def main():
    if len(sys.argv) < 2:
        print("Usage: leak-monitor.py <scan|status>")
        print("  scan    — run full CTI scan across all sources")
        print("  status  — show current findings database")
        sys.exit(1)
